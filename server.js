...
let getIngameResources = (typename, name) => {
  ...
  return rp
    .get({
      url: `http://cdn.dota2.com/apps/dota2/images/${typename}/${name}_lg.png`,
      encoding: null,
    })
    .then((image) => {
      try {
...
server.app.get("/image/hero/:hero_name.png", (req, res) =>
  getIngameResources("heroes", req.params.hero_name).then((filepath) =>
    res.sendFile(filepath)
  )
);
server.app.get("/image/item/:item_name.png", (req, res) =>
  getIngameResources("items", req.params.item_name).then((filepath) =>
    res.sendFile(filepath)
  )
);
